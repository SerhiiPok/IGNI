...
class Mdb(KaitaiStruct):
    ...
    class Face(KaitaiStruct):
        ...
        def _read(self):
            self.unknown = []
            for i in range(5):
                self.unknown.append(self._io.read_u4le())
    ...
    class Trimesh(KaitaiStruct):
        ...
        @property
        def is_skin(self):
            if hasattr(self, '_m_is_skin'):
                return self._m_is_skin

            self._m_is_skin = self._parent.node_type == Mdb.NodeType.skin
            return getattr(self, '_m_is_skin', None)
    ...
    class Header(KaitaiStruct):
        ...
        def _read(self):
            ...
            self.file_version = self._io.read_bytes(1)
            if not self.file_version == b"\x88":
                raise kaitaistruct.ValidationNotEqualError(b"\x88", self.file_version, self._io, u"/types/header/seq/1")
            ...
            self.model_name = Mdb.Strl(64, self._io, self, self._root)
            ...
            self.detail_map = Mdb.Strl(64, self._io, self, self._root)
            self.unknown_8 = self._io.read_bytes(4)
            self.model_scale = self._io.read_f4le()
            self.super_model = Mdb.Strl(60, self._io, self, self._root)
    ...
    class Material(KaitaiStruct):
        ...
        def _read(self):
            self.material_spec_lines_count = self._io.read_u4le()
            self.texture_offset = self._io.read_u4le()
            self.material_spec = []
            for i in range(self.material_spec_lines_count):
                self.material_spec.append((self._io.read_bytes_term(0, False, True, True)).decode(u"utf8"))
    ...
    class Node(KaitaiStruct):
        ...
        def _read(self):
            ...
            self.inherit_color_flag = self._io.read_bytes(4)
            self.node_id = self._io.read_u4le()
            self.node_name = Mdb.Strl(64, self._io, self, self._root)
            self.parent_geometry = self._io.read_bytes(4)
            self.parent_node = Mdb.Ptr(u"node", 32, self._io, self, self._root)
            ...
            if _on == Mdb.NodeType.trimesh:
                self.node_data = Mdb.Trimesh(self._io, self, self._root)
    ...
    def root_node(self):
        ...
        _pos = self._io.pos()
        self._io.seek((self.header.offset_root_node + 32))
        self._m_root_node = Mdb.Node(self._io, self, self._root)
        self._io.seek(_pos)
